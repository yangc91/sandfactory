...
+function ($) { "use strict";
	...
	{
		var that = this;
		//周小欠修改2014-10-22，隐藏model后，删除element
		this.$element.hide();
		this.$element.remove();
	...
	$.fn.lightbox = function (option, _relatedTarget)
	{
		return this.each(function ()
		{
			...
			var templates = '<div class="lightbox fade"  tabindex="-1" role="dialog" aria-hidden="true" >' + 
								'<div class="lightbox-dialog">' + 
									'<div class="lightbox-content">';
			if($(this).attr('img-title')) {
				templates = templates + '<div class="lightbox-caption">' + $(this).attr('img-title') + '</div>';
			}
			templates = templates  +    '<img src="' + $(this).attr('img-src') + '"></div></div></div>'; 
			var $this = $(templates);
//			$(document.body).append($this);
			...
			if (!data) {
				$this.data('bs.lightbox', (data = new Lightbox(templates, options)));
			}
